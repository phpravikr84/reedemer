...
class BridgeController extends Controller {
	...
	*/
	...
	else
	{
		...
		$offer_list=Offer::select(array('*',DB::raw("DATEDIFF(end_date,start_date) AS expires")))->where('created_by',$reedemer_id)->where('end_date','>=',$now)->with('offerDetail.inventoryDetails','campaignDetails','categoryDetails','subCategoryDetails','partnerSettings','companyDetail')->orderBy('created_at','desc')->get();
   ...
   // Show Offer Details
	...
	public function postOfferdetail(Request $request)
	{
			...
			$offer_list=Offer:: select(array('*',DB::raw("DATEDIFF(end_date,start_date) AS expires")))->where('end_date','>=',$now)->where('id',$offer_id)->with('offerDetail.inventoryDetails','campaignDetails','categoryDetails','subCategoryDetails','partnerSettings','companyDetail')->orderBy('created_at','desc')->get();
